...
def single_fisheye(PathIn, center, radius, aperture, out_pixel):
    ...
    for i in range(equirect.shape[0]):
        for j in range(equirect.shape[1]):
            x = radius * math.cos((i/out_pixel-0.5)*np.pi) * math.cos((j/out_pixel-0.5)*np.pi)
            y = radius * math.cos((i/out_pixel-0.5)*np.pi) * math.sin((j/out_pixel-0.5)*np.pi)
            z = radius * math.sin((i/out_pixel-0.5)*np.pi)
            
            if(math.acos(y/math.sqrt(x**2+y**2+z**2))/math.pi*180 > aperture/2):
                equirect[i][j]=[0, 0, 0]
                continue

            #phi를 기준으로 else 문을 만들어야 할 듯
...
if __name__ == "__main__":
    ...
    result = single_fisheye('./test_image/Car_Fisheye.jpg', [513, 497], 483, 190, 400)
    end = time.time() - start
